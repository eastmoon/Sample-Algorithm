...
package section.DEMO
{
	...
	import section.DEMO.BinarySearchTree.*;
	import section.DEMO.RedBlackTree.*;
	...
	public class Main extends MovieClip
	{
		...
		private var m_searcher : ISearcher;
		...
		private function DEMO_BinarySearchTree() : void
		{
			...
			var i = 0, j = 0, k = 0, l = 0
			var bt : ISearcher = new BSTSearcher();
			var temp : INode = null;
			...
			this.LogOut("Min : " + bt.rootNode.minimumNode.keyValue.toString() );
			this.LogOut("Max : " + bt.rootNode.maximumNode.keyValue.toString() );
		...
		{
			if( this.m_searcher == null || ( this.m_searcher != null && !(getQualifiedClassName( this.m_searcher ) == "section.DEMO.BinarySearchTree::BSTSearcher")) )
			{
				this.m_searcher = null;
				this.m_searcher = new BSTSearcher();
			}
			
			var i : Number = 0, j : Number = 0, k : Number = 0;
			var input : Array = this.mc_input.text.split(",");
			var output : Array = this.m_searcher.toArray();
			...
			if( output.length > 0 )
			{
				...
				{
					var temp : INode = this.m_searcher.Remove( output[i] );
					//if( temp != null )
			...
			for( i = 0 ; i < input.length ; i++ )
			{
				this.m_searcher.Insert( Number(input[i]) );
			}
		...
		{
			if( this.m_searcher == null || ( this.m_searcher != null && !(getQualifiedClassName( this.m_searcher ) == "section.DEMO.RedBlackTree::RBTSearcher")) )
			{
				this.m_searcher = null;
				this.m_searcher = new RBTSearcher();
			}
		...
		private function DEMO_GUI_Insert( a_event : MouseEvent ) : void
		{
			...
			this.m_searcher.Insert( Number(this.mc_insert.text) );
		...
		private function DEMO_GUI_Remove( a_event : MouseEvent ) : void
		{
			...
			this.m_searcher.Remove( Number(this.mc_remove.text) );
		...
		private function DEMO_GUI_Find( a_event : Event ) : void
		{
			if( this.m_searcher == null )
				return ;
			var node : INode = this.m_searcher.Retrieve( Number(this.mc_findLabel.text) );
		...
		private function DEMO_GUI_Successor( a_event : Event ) : void
		{
			if( this.m_searcher == null )
				return ;
			var node : INode = this.m_searcher.Retrieve( Number(this.mc_successorInput.text) );
			if( node != null )
		...
		private function DEMO_GUI_Predecessor( a_event : Event ) : void
		{
			if( this.m_searcher == null )
				return ;
			var node : INode = this.m_searcher.Retrieve( Number(this.mc_predecessorInput.text) );
			if( node != null )
		...
		private function DEMO_GUI_Draw() : void
		{
			...
			if( this.m_searcher.rootNode != null )
				this.mc_minLabel.text = this.m_searcher.rootNode.minimumNode.keyValue.toString();
			else
				this.mc_minLabel.text = "";
				
			if( this.m_searcher.rootNode != null )
				this.mc_maxLabel.text = this.m_searcher.rootNode.maximumNode.keyValue.toString();
			else
				this.mc_maxLabel.text = "";
			...
			if( this.m_searcher.rootNode != null )
				curLayer.push( this.m_searcher.rootNode );
			j = 0;
			do
			{
				k = 0;
