...
package section.DEMO.RedBlackTree
{
	...
	import section.DEMO.BinarySearchTree.BSTSearcher;
	...
	public class RBTSearcher extends BSTSearcher
	{
		 ...
		 * 複製物件；數據、連接點不可複製
		 */
		public override function CloneNode() : INode
		{
			return new RBTNode();
		}
		/*
		 * 插入數值，依據演算規則從填入適當的結構。
		 */
		public override function Insert( a_value : Object ) : INode
		{
			// 執行父級插入行為。
		 ...
		 */
		public override function Remove( a_value : Object ) : INode
		{
			// 執行父級插入行為。
			...
			if( target == null )
				child = this.rootNode as RBTNode;
			else
			{
		 ...
		 */
		private function RotateToLeft( a_node : RBTNode ) : void
		{
			var temp : RBTNode = null;
			
			// 1. Take right node is temp node.
			temp = a_node.rightNode as RBTNode;
			...
			if( temp != null )
			{
				...
				a_node.rightNode = temp.leftNode;
				if( temp.leftNode != null )
					temp.leftNode.parentNode = a_node;
				
				// 2.2 Replace temp node to this parent.
				if( a_node.parentNode != null )
					a_node.parentNode.Replace( a_node, temp );
				else
					this.rootNode = temp;

				// 2.3 Setting this parent is right node of temp node.
				temp.leftNode = a_node;
				a_node.parentNode = temp;
			}
		}
		/*
		 * 右迴旋，將當前結點右迴旋；結點的左子為新父，結點為新父的右子，新父的右子為結點的左子。
		 */
		private function RotateToRight( a_node : RBTNode ) : void
		{
			var temp : RBTNode = null;
			
			// 1. Take left node is temp node.
			temp = a_node.leftNode as RBTNode;
			
			// 2. When temp node isn't null.
			if( temp != null )
			{
				// 2.1 Replace left node to right node of temp node.
				a_node.leftNode = temp.rightNode;
				if( temp.rightNode != null )
					temp.rightNode.parentNode = a_node;
